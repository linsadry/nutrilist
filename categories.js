...
const CATEGORIES = {
  ...
  MERCEARIA: {
    ...
    items: [
      ...
      'cuscuz', 'cuscuz de milho', 'cuscuz marroquino', 'flocos de milho',
      'farinha', 'farinha de mandioca', 'farinha de trigo', 'fubá', 'fuba',
      'polvilho', 'amido de milho', 'maisena',
      // Pães e massas
...
// Prioriza a correspondência mais específica (palavra-chave mais longa),
// para que "cuscuz de milho" → Mercearia em vez de "milho" → Hortifruti.
function findCategory(itemName) {
  ...
  for (const [catKey, cat] of Object.entries(CATEGORIES)) {
    ...
    for (const keyword of cat.items) {
      ...
      if (norm === normKw) {
        matchLen = 1000; // match exato vence sempre
      } else if (norm.includes(normKw)) {
        matchLen = normKw.length; // item contém a palavra-chave
      } else if (normKw.includes(norm) && norm.length >= 4) {
        matchLen = norm.length - 1; // palavra-chave contém o item (peso menor)
      }
